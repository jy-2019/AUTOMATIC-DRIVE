...
using namespace std;
//Gamma
cv::Mat LaneDetector::gammatrans(cv::Mat inputImage) {
	cv::Mat output(inputImage.size(),CV_32FC3);
//cout<<inputImage.rows<<endl;
//cout<<inputImage.cols<<endl;
    /*Gamma校正*/
    for (int i = 0; i < inputImage.rows; i++)
		...
		//	cout<<"i:"<<i<<endl;
                ...
                {	
			//		cout<<"j:"<<j<<endl;	
                        output.at<Vec3f>(i, j)[0] = (inputImage.at<Vec3b>(i, j)[0])*(inputImage.at<Vec3b>(i, j)[0]);
                        output.at<Vec3f>(i, j)[1] = (inputImage.at<Vec3b>(i, j)[1])*(inputImage.at<Vec3b>(i, j)[1]);
                        output.at<Vec3f>(i, j)[2] = (inputImage.at<Vec3b>(i, j)[2])*(inputImage.at<Vec3b>(i, j)[2]);
                }
        }
    ...
    //cv::imshow("Gamma", output);
	return output;
}
//HOGDescriptor::gammaCorrection
...
//	cv::imshow("img_mask", img_mask);
	...
	HoughLinesP(img_mask, line, 1, CV_PI / 180, 10, 10, 0);
...
std::vector<std::vector<cv::Vec4i> > LaneDetector::lineSeparation(std::vector<cv::Vec4i> lines, cv::Mat img_edges) {
	...
	while (j < selected_lines.size()) {
		...
		if (slopes[j] > 1&&fini.x>img_center) {
			right_lines.push_back(selected_lines[j]);
			right_flag = true;
		}
		else if (slopes[j] < 0&&fini.x<img_center) {
...
std::string LaneDetector::predictTurn() {
	...
	double vanish_y;
	double thr_vp = 60;
	if(right_flag == false&&left_flag==true)
	{
		output = "Right Turnbig";
	}
	else if(right_flag == true&&left_flag==false)
	{
		output = "Left Turnbig";
	}
	else{
	...
	vanish_y = static_cast<double>(right_m*(vanish_x-right_b.x)+right_b.y);
	printf("vanish: %f\n",vanish_x);
	printf("vanish: %f\n",img_center);
	// The vanishing points location determines where is the road turning
	if (vanish_y>180&&vanish_y<360)
	{
		output = "Straight";
	}
	else{
	 if(vanish_x < (img_center - thr_vp))
		output = "Left Turnsmall";
	else if (vanish_x >(img_center + thr_vp))
		output = "Right Turnsmall";
...
cv::Mat LaneDetector::plotLane(cv::Mat inputImage, std::vector<cv::Point> lane, std::string turn) {
	...
	//cv::imwrite("detected.bmp",inputImage);
	return inputImage;
}
